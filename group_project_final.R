...
#######################################################################
############# Aditional Analysis on Verizon mentions ##################
#######################################################################

#############################################
####### Data collection: 2019 Tweets ########
#############################################


## all tweets mentions ##

Vmentions_2019_Allin <- search_tweets("@verizon", n = 20000, retryonratelimit = TRUE)

load("C:\Users\mcastilloacosta\Documents\vmentions.Rdata")
## Pre processing ##
####################

#Vmentions_clean_1 <- mutate(Vmentions_2019_Allin, text = gsub(x = text, pattern = "[0-9]+|[[:punct:]]|\\(.*\\)", replacement = ""))
Vmentions_2019_Allin$text  <-  gsub("https\\S*", "", Vmentions_2019_Allin$text)
Vmentions_2019_Allin$text  <-  gsub("@\\S*", "", Vmentions_2019_Allin$text) 
Vmentions_2019_Allin$text  <-  gsub("amp", "", Vmentions_2019_Allin$text) 
Vmentions_2019_Allin$text  <-  gsub("[\r\n]", "", Vmentions_2019_Allin$text)
Vmentions_2019_Allin$text  <-  gsub("[[:punct:]]", "", Vmentions_2019_Allin$text)
Vmentions_2019_Allin$text  <-  gsub("[0-9]", "", Vmentions_2019_Allin$text)
Vmentions_2019_Allin$text  <-  gsub("We're", "", Vmentions_2019_Allin$text)
Vmentions_2019_Allin$text  <-  gsub("we're", "", Vmentions_2019_Allin$text)
Vmentions_2019_Allin$text  <-  gsub("we've", "", Vmentions_2019_Allin$text)
Vmentions_2019_Allin$text  <-  gsub("rt", "", Vmentions_2019_Allin$text)
Vmentions_2019_Allin$text  <-  gsub("g", "", Vmentions_2019_Allin$text)
Vmentions_clean_1 <-Vmentions_2019_Allin

# 2. Tokenization (+ going to lowercase)
Vmentions_clean_2 <- Vmentions_clean_1 %>% unnest_tokens(output = "word", # how should the new column be named?
...
Vmentions_clean_3 <- filter(Vmentions_clean_2, substr(word, 1, 1) != '#', 
                            substr(word, 1, 1) != '@') # This compares for the first letter of a token# omit hashtags
...
#}
#Vmentions_clean_4 <- Vmentions_clean_3  %>%  mutate(suggestion = correct_spelling(word))


# 5. remove stopwords

Vmentions_clean_4 <- Vmentions_clean_3 %>% anti_join(get_stopwords()) # note that I continue with the 'uncorrected' words here
# this strongly reduces the number of words

# 6. Selecting only tweets in English
Vmentions_clean_5 <- Vmentions_clean_4[Vmentions_clean_4$lang == "en", ]

# 7. Lemmatization
#install.packages("textstem")
#library(textstem)
stem_words = lemmatize_words(Vmentions_clean_5$word, dictionary= lexicon::hash_lemmas)
Vmentions_clean_6 <- cbind(stem_words, Vmentions_clean_5)

# 8. Additional cleanning
Vmentions_clean_7 <- iconv(Vmentions_clean_6, from = 'UTF-8', to = 'ASCII', sub = "")


# Data Frame #
Vmentions_cleanDF<- as.data.frame(Vmentions_clean_6)

# Data Term Matrix #
VmentionsDTM <- Vmentions_clean_7 %>% count(status_id, word, sort = TRUE) %>% cast_dtm( word,n,weighting = tm::weightTfIdf)

VmentionsDTMDense <- removeSparseTerms(VmentionsDTM,0.7)

####################################################

rt <- lat_lng(Vmentions_clean_4)
...
#### Summary statistics of TMLS- Verizon tweets ####
mean(tmls$favorite_count)
max(tmls$favorite_count)
min(tmls$favorite_count)
t <- tmls[tmls$favorite_count == 28715,]
print(t)
## What does the max tweet said? ##
subset(tmls$text, tmls$favorite_count == 28715)

#### Summary statistics of Verizon_Tweets_reult- Verizon tweets ####
mean(Verizon_Tweets_result$favorite_count)
max(Verizon_Tweets_result$favorite_count)
max(Verizon_Tweets_result$reply_count)

max(rt_origin$reply_count)
...
Negative_replies<- count(reply_analysis[reply_analysis$result == 0,])
print(Negative_replies)

positive_replies<- count(reply_analysis[reply_analysis$result == 4,])
print(positive_replies)
...
# Vmentions_cleanandEng_count_5 /  vmention
...
vmentionsSentiment <- inner_join(Vmentions_clean_4,get_sentiments("nrc"))

sum(unique(vmentionsSentiment$sentiment, ))

summarySentiment_vmentions <- vmentionsSentiment %>%  count(word,sentiment,sort=TRUE) %>%
  group_by(sentiment) %>%
  top_n(10) %>%  
  arrange(n) %>%
  as.data.frame(stringsAsFactors=FALSE)

## Solving error of " plot region too large"
dev.off()
###########################################

par(oma=c(1,1,1,1),mfrow=c(1,2))
barplot(summarySentiment_vmentions[summarySentiment_vmentions$sentiment == "negative",3],names.arg = summarySentiment_vmentions[summarySentiment_vmentions$sentiment == "negative",1],horiz=TRUE,las=1,xlim=c(0,15),col="red",axes=TRUE)                
barplot(summarySentiment_vmentions[summarySentiment_vmentions$sentiment == "positive",3],names.arg = summarySentiment_vmentions[summarySentiment_vmentions$sentiment == "positive",1],horiz=TRUE,las=1,xlim=c(0,15),col="green")                
#barplot(summarySentiment_vmentions[summarySentiment_vmentions$sentiment == "trust",3],names.arg = summarySentiment_vmentions[summarySentiment_vmentions$sentiment == "trust",1],horiz=TRUE,las=1,xlim=c(0,15),col="blue")                
#barplot(summarySentiment_vmentions[summarySentiment_vmentions$sentiment == "fear",3],names.arg = summarySentiment_vmentions[summarySentiment_vmentions$sentiment == "fear",1],horiz=TRUE,las=1,xlim=c(0,15),col="yellow")                

#par(mar=c(1,1,1,1,))

########## Sentiment with different dictionary #####

get_sentiments("bing") %>% 
  count(sentiment)

vmentionsSentiment2 <- inner_join(Vmentions_clean_4,get_sentiments("bing"))

summarySentiment_vmentions2 <- vmentionsSentiment2 %>%  count(word,sentiment,sort=TRUE) %>%
  group_by(sentiment) %>%
  top_n(10) %>%  
  arrange(n) %>%
  as.data.frame(stringsAsFactors=FALSE)
...
barplot(summarySentiment_vmentions2[summarySentiment_vmentions2$sentiment == "negative",3],names.arg = summarySentiment_vmentions2[summarySentiment_vmentions2$sentiment == "negative",1],horiz=TRUE,las=1,xlim=c(0,160),col="red",axes=TRUE)                
barplot(summarySentiment_vmentions2[summarySentiment_vmentions2$sentiment == "positive",3],names.arg = summarySentiment_vmentions2[summarySentiment_vmentions2$sentiment == "positive",1],horiz=TRUE,las=1,xlim=c(0,300),col="green")                


if (!require("ggplot2")) install.packages("ggplot2", quiet=TRUE) ; require("ggplot2")

summarySentiment_vmentions2 %>%
  ungroup() %>%
  mutate(word = reorder(word, n)) %>%
  ggplot(aes(word, n, fill = sentiment)) +
  geom_col(show.legend = FALSE) +
...
statusSentiment <- vmentionsSentiment2 %>%
...
statusSentiment2 <- inner_join(Vmentions_clean_4,get_sentiments("afinn")) %>%
  group_by(status_id) %>%                      # here we get numeric values, so we can just sum them per post
  summarize(Sentiment = sum(value)) 
# here we also have 73 posts that are labeled
mean(statusSentiment2$Sentiment)


##########################################################################
######################### Verizon_Tweets_result ##########################
##########################################################################

Verizon_Tweets_result <- mutate(Verizon_Tweets_result, text = gsub(x = text, pattern = "[0-9]+|[[:punct:]]|\\(.*\\)", replacement = ""))



Verizon_Tweets_result_Tokenized <-  Verizon_Tweets_result %>% unnest_tokens(output = "word", # how should the new column be named?
                                                                            input = text, # where can we find the text? 
                                                                            token = "words", # which tokenization scheme should we follow?
                                                                            drop=FALSE,to_lower=TRUE) # drop=FALSE specifies that we want to keep our text; to_lower puts everyting to lowercase

Verizon_Tweets_result_Tokenized <- filter(Verizon_Tweets_result_Tokenized, substr(word, 1, 1) != '#', 
                                          substr(word, 1, 1) != '@') # This compares for the first letter of a token# omit hashtags


Verizon_Tweets_result_Tokenized <- Verizon_Tweets_result_Tokenized %>% anti_join(get_stopwords())
#save(Verizon_Tweets_result_Tokenized, file = "Verizon_Tweets_result_Tokenized.Rdata")

VmentionsTweetsResultFreq <- Verizon_Tweets_result_Tokenized %>% group_by(word) %>% # for this, we need to have the sum over all documents
  summarize(freq = n()) %>%
  arrange(-freq)  
#save(VmentionsTweetsResultFreq, file = "VmentionsTweetsResultFreq.Rdata")



wordcloud(VmentionsTweetsResultFreq$word, VmentionsTweetsResultFreq$freq,
          max.words=80,
          scale=c(3,1))
...
VerizonTweetsResult_Sentiment <- inner_join(Verizon_Tweets_result_Tokenized,get_sentiments("bing"))

summaryVerizonTweetsResult_Sentiment <- VerizonTweetsResult_Sentiment %>%  count(word,sentiment,sort=TRUE) %>%
  group_by(sentiment) %>%
  top_n(10) %>%  
  arrange(n) %>%
  as.data.frame(stringsAsFactors=FALSE)
...
par(oma=c(0,4,0,0),mfrow=c(1,2))
barplot(summaryVerizonTweetsResult_Sentiment[summaryVerizonTweetsResult_Sentiment$sentiment == "negative",3],names.arg = summaryVerizonTweetsResult_Sentiment[summaryVerizonTweetsResult_Sentiment$sentiment == "negative",1],horiz=TRUE,las=1,xlim=c(0,40),col="red",axes=TRUE)                
barplot(summaryVerizonTweetsResult_Sentiment[summaryVerizonTweetsResult_Sentiment$sentiment == "positive",3],names.arg = summaryVerizonTweetsResult_Sentiment[summaryVerizonTweetsResult_Sentiment$sentiment == "positive",1],horiz=TRUE,las=1,xlim=c(0,300),col="green")                
